...
def ptv_gap_fraction(cumulative_dose: np.ndarray,
                     prescription_volume: np.ndarray) -> float:
    """Mean unmet PTV fraction in ``[0, 1]`` (0 = every PTV voxel at Rx).

    ``mean over PTV voxels of clip(Rx - cumulative_dose, 0) / Rx``.
    """
    ptv_voxels = prescription_volume > 0
    if not ptv_voxels.any():
        return 0.0
    gap = np.clip(
        prescription_volume[ptv_voxels] - cumulative_dose[ptv_voxels], 0.0, None
    )
    return float((gap / prescription_volume[ptv_voxels]).mean())
...
def oar_overshoot_fraction(cumulative_dose: np.ndarray,
    ...
    For each OAR: ``max(0, mean_dose - tol) / tol``, scaled by its weight
    and summed. 0 while the cumulative dose stays under budget; grows once
    an organ exceeds its whole-course tolerance.
    """
    organ_weights = organ_weights or {}
    total = 0.0
    for organ_name, tolerance in full_course_tolerances.items():
        ...
        overshoot = max(0.0, (mean_dose - tolerance) / tolerance)
        total += float(organ_weights.get(organ_name, 1.0)) * overshoot
    return total
...
def sequential_potential(cumulative_dose: np.ndarray,
    ...
    """
    ptv_term = ptv_gap_fraction(cumulative_dose, prescription_volume)
...
def terminal_reward(cumulative_dose: np.ndarray,
                    ...
                    *,
                    lambda_ptv: float,
                    terminal_dvh_weight: float) -> float:
    ...
    for ptv_name, rx in prescriptions.items():
        ...
        if mask is None or mask.sum() == 0:
            continue
        coverages.append(coverage(cumulative_dose[mask > 0], rx))
